...
class Mixin:
    ...
    def PrototypeExternalEntanglementSwapping(self, process, edge1, edge2, num_required=1, label_in='Physical', label_out='Physical', resource_type='Physical'):
        ...
        while process['isSuccess'] < num_required: 
            ...
            # Separate here?
            info = (event, leftNode, swapper, rightNode, table, resource_type, label_out, num_required, process)

            self.env.process(self._independentES(info))

    def _independentES(self, info):

        event, leftNode, swapper, rightNode, table, resource_type, label_out, num_required, process = info
